...
function GetTeamId() {
    $.ajax({
        ...
        success: function (data) {
            TeamId = data;
			setMenu(TeamId,'reportsmartteam');
        }
    });
}
GetTeamId();
...
function GetImgGeoCode(GeoCode, GeoName) {
    ...
    $("#head_vncode").html('');
    $("#head_vncode").append('BỘ TIÊU CHÍ ĐÁNH GIÁ CÔNG TÁC ĐOÀN VÀ PHONG TRÀO THANH THIẾU NHI THÀNH ĐOÀN ' + GeoName.toUpperCase() + ' NĂM 2018');
}
...
function DrawPointTable() {
    ...
    var TeamId = cboTeam.selectedIndex;
    var param = ContentId != '' ? '&contentid=' + ContentId : '';
    $('#btcPointTable tbody').html('');
    $.ajax({
        type: 'POST',
        dataType: "json",
        url: "/cms/getBTCPointSmartTeam?teamid=" + TeamId + param,
        data: '',
        ...
        success: function (data) {
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    tr_html += '<td>' + (objBTCPointSmart.Type == 'Cấp tỉnh' ? objBTCPointSmart.Provincial : objBTCPointSmart.Type) + '</td>';
...
function LoadDataByProvincal() {
    ...
    var t = document.getElementById("cboTeam");
    var team = t.options[t.selectedIndex].value;
    if (t.selectedIndex == 0) {
        AlertAutoCloseError("Bạn hãy chọn ban muốn xem");
        return;
    }		
